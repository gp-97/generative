...
fn test_rotation() {
    ...
    let mut i = 0.0;
    while i <= 360.0 {
        let (x_pxl, y_pxl) = transforms::rotate(&canvas, x_pxl, y_pxl, 511.0, 511.0, Angle::DEGREE(-1.0 * i as f32));
...
fn test_shear_x() {
    ...
    while i <= 360.0 {
        ...
        canvas.set_pixel_at(x_pxl as usize, y_pxl as usize, (10, 156, 210, 255));
        let (x_pxl, y_pxl) = transforms::shear_x(&canvas, x_pxl, y_pxl, 0.85);
        canvas.set_pixel_at(x_pxl as usize, y_pxl as usize, (180, 156, 10, 255));
        i += 0.1;
    }
    save_image(Canvas::to_photon(&canvas), "assets/canvas_shear_x.png");
}
#[test]
fn test_shear_y() {
    let mut canvas = Canvas::new(1024, 1024);
    ...
    while i <= 360.0 {
        ...
        canvas.set_pixel_at(x_pxl as usize, y_pxl as usize, (10, 156, 210, 255));
        let (x_pxl, y_pxl) = transforms::shear_y(&canvas, x_pxl, y_pxl, 0.85);
        canvas.set_pixel_at(x_pxl as usize, y_pxl as usize, (180, 156, 10, 255));
        i += 0.1;
    }
    save_image(Canvas::to_photon(&canvas), "assets/canvas_shear_y.png");
}
